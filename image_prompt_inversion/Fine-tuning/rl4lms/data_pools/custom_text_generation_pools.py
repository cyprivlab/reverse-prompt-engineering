...
class AnswerToQuestionPool(TextGenPool):
    @classmethod
    def normalize_text(cls, text, strip: bool = True):
        # 标准化文本
        text = text.lower()
        if strip:
            text = text.strip()
        text = text.replace("'", '')  # 正确赋值回 text
        return text
    ...
    def prepare(cls, split: str):
        ...
        # 读取数据
        split_df = pandas.read_csv(split_path, sep='\t', header=None, names=["answer", "question"], encoding="utf-8")

        # # 创建问题到答案的映射
        # question_to_answers = defaultdict(list)
        # for _, row in split_df.iterrows():
        #     question_to_answers[row['question']].append(row['answer'])

        # # 创建样本集
        # samples = []
        # for ix, (question, answers) in enumerate(question_to_answers.items()):
        #     sample = Sample(
        #         id=f"{split}_{ix}",
        #         prompt_or_input_text=cls.normalize_text(question),  # 将问题作为输入
        #         references=[cls.normalize_text(answer) for answer in answers]  # 所有答案作为多个输出
        #     )
        #     samples.append(sample)
        ...
        for ix, row in split_df.iterrows():
            sample = Sample(
                id=f"{split}_{ix}",
                prompt_or_input_text=cls.normalize_text(row['answer']),  # 将答案作为输入
                references=[cls.normalize_text(row['question'])]  # 将问题作为输出
            )
            samples.append(sample)
...
class AlpacaGPT4(TextGenPool):
    ...
    def prepare(cls,split: str='train', prompt_suffix: str = "", prompt_prefix: str = ""):
        ...
        for ix, item in tqdm(enumerate(dataset),
                             desc="Preparing dataset",
                             total=len(dataset)):
            enquiry =  cls.normalize_text(item["instruction"] + item['input'])
            answer = cls.normalize_text(item["output"])  # Assuming each item in the dataset includes a correct question to be generated.

            sample = Sample(id=f"{split}_{ix}",
                            prompt_or_input_text=answer,
                            references=[enquiry])
            samples.append(sample)


        pool_instance = cls(samples)
        print("Dataset is prepared successfully, containing {} samples.".format(len(samples)))
        return pool_instance

class GPTRQA(TextGenPool):
    @classmethod
    def normalize_text(cls, text, strip: bool = True):
        # 标准化文本
        text = text.lower()
    ...
    def prepare(cls,split: str='train', prompt_suffix: str = "", prompt_prefix: str = ""):
        ...
        for ix, item in tqdm(enumerate(dataset),
                             desc="Preparing dataset",
                             total=len(dataset)):
            enquiry =  cls.normalize_text(item['question'])
            answer = cls.normalize_text(item["answer"])  # Assuming each item in the dataset includes a correct question to be generated.
...
class MediData(TextGenPool):
    ...
    def prepare(cls,split: str='train', prompt_suffix: str = "", prompt_prefix: str = ""):
        ...
        for ix, item in tqdm(enumerate(dataset),
                             desc="Preparing dataset",
                             total=len(dataset)):
            enquiry =  cls.normalize_text(('enquiry: ' + item['input']))
            answer = cls.normalize_text(('answer: '+ item["output"]))  # Assuming each item in the dataset includes a correct question to be generated.
...
class COCO(TextGenPool):
    ...
    def prepare(cls,split: str='train', prompt_suffix: str = "", prompt_prefix: str = ""):
        ...
        for ix, item in tqdm(enumerate(dataset),
                             desc="Preparing dataset",
                             total=len(dataset)):
            enquiry =  cls.normalize_text(item['target_text'])
            answer = cls.normalize_text(item["input_text"])  # Assuming each item in the dataset includes a correct question to be generated.
...
class SquadQA(TextGenPool):
    ...
    def prepare(cls,split: str='train', prompt_suffix: str = "", prompt_prefix: str = ""):
        ...
        dataset = dataset['train'].shuffle().select(range(20000))
        if dataset is None:
            print("Dataset loading failed.")
            return None
        ...
        for ix, item in tqdm(enumerate(dataset),
                             desc="Preparing dataset",
                             total=len(dataset)):
            enquiry =  cls.normalize_text("question: "+ item["question"])
            answer = cls.normalize_text(" answer: " + item['answers']['text'][0])  # Assuming each item in the dataset includes a correct question to be generated.
