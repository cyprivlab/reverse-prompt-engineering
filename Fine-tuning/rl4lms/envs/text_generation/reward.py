...
class ModifyBERTScoreRewardFunction(RewardFunction):
    def __init__(self, language: str = "en") -> None:
        super().__init__()
        self._metric = BERTScoreMetric(language)
    ...
    def __call__(
        ...
        if done:
            ...
            bert_score = metric_results["semantic/bert_score"][1]
            if bert_score > 0.5:
                return bert_score*10
            else:
                return (1-bert_score)*(-10)
        return 0
...
class ModifyBLEURewardFunction(RewardFunction):
    def __init__(self) -> None:
        super().__init__()
        self._metric = BLEUMetric()
...
class CosineSimilarityReward(RewardFunction):
    def __init__(self) -> None:
        super().__init__()

    # def get_embeddings(self, texts):
    #     cleaned_list = texts.replace("\n", " ")
    #     api_k="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"
    #     client = openai.OpenAI(api_key=api_k)
    ...
    def get_embeddings(self, texts):
        cleaned_list = texts.replace("\n", " ")
        encor = SentenceTransformer('all-MiniLM-L6-v2')
        response=encor.encode(cleaned_list, convert_to_tensor=True)
        return response
    ...
    def __call__(self, current_observation: Observation, action: int,
                 next_observation: Observation, done: bool,
                 meta_info: Dict[str, Any] = None) -> float:
        if done:
            print("!!!!!next observation.context: ", next_observation.context_text)
            print("!!!!!next observation.target_references: ", next_observation.target_or_reference_texts)

            # Assuming that 'context_text' is the generated text
            generated_text = next_observation.context_text
            # Assuming there's a reference text to compare against
            reference_text = next_observation.target_or_reference_texts[0]
            if generated_text == ' ' or '' or '  ' or '   ':
                generated_text = 'None'
            # Get embeddings
            gen_embedding = self.get_embeddings(generated_text)
            ref_embedding = self.get_embeddings(reference_text)

            # Compute cosine similarity as the reward
            score = self.cosine_similarity(gen_embedding, ref_embedding)
            print('Use cosine similarity as reward: ', score)
            if score > 0.5:
                return score*10
            else:
                return (1-score)*(-10)
            
        return 0  # Return zero reward if the episode is not done
...
if __name__ == "__main__":
    ...
    reward_fn = CosineSimilarityReward()
    print(reward_fn(None, None, observation, True))
